...
pub fn sendConfig(self: *hwc.OutputManager) !void {
...
pub fn finishConfiguration(self: *hwc.OutputManager) void {
...
pub fn cancelConfiguration(self: *hwc.OutputManager) void {
    ...
    while (iterator.next()) |head| {
        const output: *hwc.Output = @ptrFromInt(head.state.output.data);
...
fn handleApply(
    ...
    const manager: *hwc.OutputManager = @fieldParentPtr("apply_config", listener);
...
fn handleDestroy(
    ...
    const manager: *hwc.OutputManager = @fieldParentPtr("destroy", listener);
